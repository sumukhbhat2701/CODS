...
function countryData(){
    ...
    for(let i=0;i<Object.keys(data).length;i++){
        ...
        <strong><em>${country}:</em></strong><br>
        <strong>Confirmed Cases </strong>
        ${confirmedCases}<br>
                    <strong style='color:red'>Deaths: </strong>
                    ${deadCases}<br>
                    <strong style='color:green'>Recovered Cases: </strong>
                    ${recoveredCases}<br>

                    <strong>Active Cases: </strong>${activeCases}<br>
